...
impl Gte {
    ...
    fn command_mvmva(&mut self) {
        ...
        let mx11 = mx.m11 as i64; let mx12 = mx.m12 as i64; let mx13 = mx.m13 as i64;
        let mx21 = mx.m21 as i64; let mx22 = mx.m22 as i64; let mx23 = mx.m23 as i64;
        let mx31 = mx.m31 as i64; let mx32 = mx.m32 as i64; let mx33 = mx.m33 as i64;
        ...
        let trx = (tx as i64) << 12;
        let try = (ty as i64) << 12;
        let trz = (tz as i64) << 12;

        self.mac[1] = (self.a(1, trx + mx11 * vx + mx12 * vy + mx13 * vz) >> self.sf) as i32;
        self.mac[2] = (self.a(2, try + mx21 * vx + mx22 * vy + mx23 * vz) >> self.sf) as i32;
        self.mac[3] = (self.a(3, trz + mx31 * vx + mx32 * vy + mx33 * vz) >> self.sf) as i32;
        ...
        if self.cv == 2 {
            self.mac[1] = (self.a(1, trx + mx11 * vx) >> self.sf) as i32;
            self.mac[2] = (self.a(2, try + mx21 * vx) >> self.sf) as i32;
            self.mac[3] = (self.a(3, trz + mx31 * vx) >> self.sf) as i32;
            ...
            self.mac[1] = (self.a(1, mx12 * vy + mx13 * vz) >> self.sf) as i32;
            self.mac[2] = (self.a(2, mx22 * vy + mx23 * vz) >> self.sf) as i32;
            self.mac[3] = (self.a(3, mx32 * vy + mx33 * vz) >> self.sf) as i32;
    ...
    fn command_cc(&mut self) {
        ...
        let l11 = self.light.m11 as i64; let l12 = self.light.m12 as i64; let l13 = self.light.m13 as i64;
        let l21 = self.light.m21 as i64; let l22 = self.light.m22 as i64; let l23 = self.light.m23 as i64;
        let l31 = self.light.m31 as i64; let l32 = self.light.m32 as i64; let l33 = self.light.m33 as i64;
        ...
        self.mac[1] = (self.a(1, rbk + l11 * ir1 + l12 * ir2 + l13 * ir3) >> self.sf) as i32;
        self.mac[2] = (self.a(2, gbk + l21 * ir1 + l22 * ir2 + l23 * ir3) >> self.sf) as i32;
        self.mac[3] = (self.a(3, bbk + l31 * ir1 + l32 * ir2 + l33 * ir3) >> self.sf) as i32;
    ...
    fn command_cdp(&mut self) {
        ...
        self.mac[1] = (self.a(1, mac1 + (rfc - mac1) * ir0) >> self.sf) as i32;
        self.mac[2] = (self.a(2, mac2 + (gfc - mac2) * ir0) >> self.sf) as i32;
        self.mac[3] = (self.a(3, mac3 + (bfc - mac3) * ir0) >> self.sf) as i32;

        let mac1 = self.mac[1] as i32;
        let mac2 = self.mac[2] as i32;
        let mac3 = self.mac[3] as i32;
    ...
    fn command_dcpl(&mut self) {
        ...
        let mac1 = self.mac[1] as i64;
        let mac2 = self.mac[1] as i64;
        let mac3 = self.mac[1] as i64;
    ...
    fn command_gpl(&mut self) {
        ...
        self.mac[1] = (self.a(1, ir0 * ir1 + mac1) as i32) >> self.sf;
        self.mac[2] = (self.a(2, ir0 * ir2 + mac2) as i32) >> self.sf;
        self.mac[3] = (self.a(3, ir0 * ir3 + mac3) as i32) >> self.sf;
    ...
    fn rtp(&mut self, index: usize) {
        ...
        let trx = (self.tr.x as i64) << 12;
        let try = (self.tr.y as i64) << 12;
        let mut trz = (self.tr.z as i64) << 12;

        let r11 = self.rotation.m11 as i64; let r12 = self.rotation.m12 as i64; let r13 = self.rotation.m13 as i64;
        let r21 = self.rotation.m21 as i64; let r22 = self.rotation.m22 as i64; let r23 = self.rotation.m23 as i64;
        let r31 = self.rotation.m31 as i64; let r32 = self.rotation.m32 as i64; let r33 = self.rotation.m33 as i64;

        let vx = self.v[index].x as i64;
        let vy = self.v[index].y as i64;
        let vz = self.v[index].z as i64;

        self.mac[1] = (self.a(1, trx + r11 * vx + r12 * vy + r13 * vz) >> sf) as i32;
        self.mac[2] = (self.a(2, try + r21 * vx + r22 * vy + r23 * vz) >> sf) as i32;

        trz = self.a(3, trz + r31 * vx + r32 * vy + r33 * vz);
        self.mac[3] = (trz >> sf) as i32;
    ...
    fn nc(&mut self, index: usize) {
        ...
        self.mac[1] = (self.a(1, l11 * vx + l12 * vy + l13 * vz) >> self.sf) as i32;
        self.mac[2] = (self.a(2, l21 * vx + l22 * vy + l23 * vz) >> self.sf) as i32;
        self.mac[3] = (self.a(3, l31 * vx + l32 * vy + l33 * vz) >> self.sf) as i32;
        ...
        let c11 = self.colour.m11 as i64; let c12 = self.colour.m12 as i64; let c13 = self.colour.m13 as i64;
        let c21 = self.colour.m21 as i64; let c22 = self.colour.m22 as i64; let c23 = self.colour.m23 as i64;
        let c31 = self.colour.m31 as i64; let c32 = self.colour.m32 as i64; let c33 = self.colour.m33 as i64;

        self.mac[1] = (self.a(1, rbk + c11 * ir1 + c12 * ir2 + c13 * ir3) >> self.sf) as i32;
        self.mac[2] = (self.a(2, gbk + c21 * ir1 + c22 * ir2 + c23 * ir3) >> self.sf) as i32;
        self.mac[3] = (self.a(3, bbk + c31 * ir1 + c32 * ir2 + c33 * ir3) >> self.sf) as i32;
    ...
    pub fn divide(numerator: u16, divisor: u16) -> u32 {
        ...
        let reciprocal = Gte::reciprocal(d) as u64;
    ...
    pub fn read_control(&self, index: usize) -> u32 {
        match index {
            ...
            30 => self.zsf4 as u32,
            31 => self.flags,
            _ => panic!("[GTE] [ERROR] Read from unknown control register cop2r{}", index),
        }
    }
    ...
    pub fn read_data(&self, index: usize) -> u32 {
        match index {
            ...
            30 => self.lzcs as u32,
            31 => self.lzcr as u32,
            _ => panic!("[GTE] [ERROR] Read from unknown data register cop2r{}", index),
        }
    }
    ...
    pub fn write_data(&mut self, index: usize, value: u32) {
        match index {
            ...
            31 => (),
            _ => panic!("[GTE] [ERROR] Write to unknown data register cop2r{}", index),
        }
    }
}